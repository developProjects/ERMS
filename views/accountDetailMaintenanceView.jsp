...
<script>
 				...
 				function(){
					...
					*/
					...
					var dataSource = new kendo.data.DataSource({
						transport: {
							read: 
								url: window.sessionStorage.getItem('serverPath')+"acct/getRecord?acctId="+paraAcctId+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+getURLParameters("bizUnit")+"&bookEntity="+getURLParameters("bookEntity")+"&dataSourceAppId="+getURLParameters("dataSourceAppId"),
								dataType: "json",
			    			    xhrFields: {
	...
	/* Action - Save button */
	function toUpdate(acctId, bizUnit, dataSourceAppId, userId, bookEntity){
		window.location = "/ermsweb/viewAccountsUpdate?acctId="+acctId+"&bizUnit="+bizUnit+"&dataSourceAppId="+dataSourceAppId+"&bookEntity="+bookEntity;
	}
...
<body>
	<div class="boci-wrapper">
		...
		<div class="page-title">Account RMD Maintenance - View </div>
<!-- 		<input type="hidden" id="pagetitle" name="pagetitle"
			value="Account RMD Maintenance - View "> -->
				...
				<tbody>
					...
					<tr>
						<td style="font-size: 13px; background-color: #B5A2C6">
							style="font-size:13px; background-color:#B5A2C6"Account - Name.</td>
