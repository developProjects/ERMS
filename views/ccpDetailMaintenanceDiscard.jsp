...
<script>
			...
			*/
			...
			var dataSource = new kendo.data.DataSource({
				transport: {
					...
					update: {
						type:"POST",
						url: window.sessionStorage.getItem('serverPath')+"legalParty/discard",
						dataType: "json",
						xhrFields: {
				    		withCredentials: true
				    	},
			...
			dataSource.fetch(function(){
				...
				$.each(jsonObj, function(i){
						...
						$("#financial_report_date_label").html("<label>"+validateResponse(jsonObj[i].latestFinRptDate)+"</label>");
						...
						$("#buttonType").html("<button id=\"yesBtn\"  type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\">YES</button><label>    </label><button id=\"backBtn\" onclick=\"toBack()\" type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\">NO</button>");   								
		...
		function toUpdate(uid){
			window.location = "/ermsweb/viewCrUpdate?ccdId="+uid;
		}
		
		/* Action - Update button */
		function toDiscard(uid){
			window.location = "/ermsweb/viewDiscardCounterParty?ccdId="+uid;
		}
...
<body>
	...
	<div class="page-title">Client Counterparty RMD Detail Maintenance - Discard Client Counterparty Change Request</div>
	
	<!-- <input type="hidden" id="pagetitle" name="pagetitle"
		value="Client Counterparty RMD Detail section" />
 -->	<table id="listTable" cellpadding="6" cellspacing="1" width="500"
