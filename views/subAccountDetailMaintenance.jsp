...
<script type="text/javascript">
	// used to sync the exports
	  ...
	  var getAccountsCrDetailsURL = window.sessionStorage.getItem('serverPath')+"subacct/getDetailsChangeRequest?userId="+window.sessionStorage.getItem("username");

	  var getAccountDetailsURL = window.sessionStorage.getItem('serverPath')+"subacct/getDetails?userId="+window.sessionStorage.getItem("username");
	...
	function dataGrids(){
		...
		var activeDataSource = new kendo.data.DataSource({
			...
			error:function(e){
				if(e.xhr.status == "401"){
					...
					window.parent.location = "/ermsweb/home";
					}else{
						alert(e.errorThrown);
					}
			},
		...
		$("#active-account-list").kendoGrid({
			...
			columns: [
				...
				{ field: "ccdId", title:"CCDID", autoWidth:true},
				{ field: "cmdCustId",title:"CMD Client ID",autoWidth:true},
				{ field: "subAcc",title:"Sub Account No.",autoWidth:true},
				{ field: "legalPartyName", title:"Client/Counterparty Name", autoWidth:true},
				{ field: "acctId",title:"Account ID",autoWidth:true},
				...
				{ field: "lastUpdateBy", title:"Update By", autoWidth:true},
				{ field: "lastUpdateDt",title:"Last Update time", autoWidth:true, template: "#=toDateFormat(lastUpdateDt)#"},
				{ field: "action",title:"Action", template:"#=replaceActionGraphics('|',false, action, acctId, subAcc , bizUnit, bookEntity, dataSourceAppId)#", autoWidth:true}					
			]	
		});
		...
		$("#cr-account-list").kendoGrid({
				...
				columns: [
					...
					{ field: "crBy", title:"Updated By", autoWidth:true},
					{ field: "action", title:"Action",template:"#=replaceActionGraphics('|',true , action, acctId, subAcc, bizUnit, bookEntity, dataSourceAppId, crId)#", autoWidth:true}					
				]
		});
		closeModal();
	}
	...
	function replaceActionGraphics(delim, crStatus, data, acctId, subAcc , bizUnit, bookEntity, dataSourceAppId, crId){
		if(data!=null){
			var spltarr = data;
			var split_string = "";
			...
			if(spltarr != "" && spltarr != undefined && spltarr != null){
				if (new RegExp("V").test(spltarr)){
						if(!crStatus){
							split_string = split_string+"<a href='/ermsweb/viewSubAccountDetails?acctId="+acctId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+bizUnit+"&bookEntity="+bookEntity+"&dataSourceAppId="+dataSourceAppId+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_view.png' width=\"20\" height=\"20\"/></a>";
						}else{
							split_string = split_string+"<a href='/ermsweb/viewCrSubAccounts?crId="+crId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_view.png' width=\"20\" height=\"20\"/></a>";
						}
				}
				if (new RegExp("E").test(spltarr)){
					if(!crStatus){
						split_string = split_string+"<a href='/ermsweb/viewSubAccountsUpdate?acctId="+acctId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+bizUnit+"&bookEntity="+bookEntity+"&dataSourceAppId="+dataSourceAppId+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_update.png' width=\"20\" height=\"20\"/></a>";
					}else{
						split_string = split_string+"<a href='/ermsweb/viewCrSubAccountsUpdate?crId="+crId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+bizUnit+"&bookEntity="+bookEntity+"&dataSourceAppId="+dataSourceAppId+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_update.png' width=\"20\" height=\"20\"/></a>";
					}
				}
				if (new RegExp("D").test(spltarr)){
					split_string = split_string+"<a href='/ermsweb/discardSubAccounts?crId="+crId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+bizUnit+"&bookEntity="+bookEntity+"&dataSourceAppId="+dataSourceAppId+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_discard2.png' width=\"20\" height=\"20\"/></a>";
				}
				if (new RegExp("A").test(spltarr)){
					split_string = split_string+"<a href='/ermsweb/verifySubAccounts?crId="+crId+"&subAcc="+subAcc+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+bizUnit+"&bookEntity="+bookEntity+"&dataSourceAppId="+dataSourceAppId+"'><img style=\"border-style: none\" src='/ermsweb/resources/images/bg_update.png' width=\"20\" height=\"20\"/></a>";
				}
...
<body onload="checkSessionAlive()">
	<div class="boci-wrapper">
		...
		<div class="page-title">Sub Account RMD Detail Maintenance</div>
		<div id="boci-content-wrapper">
			<!-- <input type="hidden" id="pagetitle" name="pagetitle"
				value="Sub Account RMD Detail Maintenance"> -->
					...
					<tbody id="search-filter-body" style="display: block">
						...
						<tr>
							...
							<div id="result2Title" style="background-color: #9C3E3E; color: white; display: none;">
								<label>&nbsp;&nbsp;&nbsp;</label>Sub-Account RMD Change Request
								Table
							</div>
