...
<script>
		$(document).ready(
 				function(){
					...
					var dataSource = new kendo.data.DataSource({
						transport: {
							read: {
								url: window.sessionStorage.getItem('serverPath')+"subacct/getRecord?acctId="+paraAcctId+"&subAcc="+getURLParameters("subAcc")+"&userId="+window.sessionStorage.getItem("username")+"&bizUnit="+getURLParameters("bizUnit")+"&bookEntity="+getURLParameters("bookEntity")+"&dataSourceAppId="+getURLParameters("dataSourceAppId"),
								dataType: "json",
					...
					dataSource.fetch(function(){
						...
						$.each(jsonObj, function(i){
							...
							$("#buttonType").html("<button id=\"updateBtn\" type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\" onclick=\"toUpdate('"+jsonObj[i].acctId+"','"+jsonObj[i].subAcctId+"','"+jsonObj[i].bizUnit+"','"+jsonObj[i].dataSourceAppId+"','"+getURLParameters("userId")+"', '"+jsonObj[i].bookEntity+"')\">Update</button><label>    </label><button id=\"backBtn\" onclick=\"toBack()\" type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\">Back</button>");
						});
					});
				});		
	...
	/* Action - Save button */
	function toUpdate(acctId, subAcc, bizUnit, dataSourceAppId, userId, bookEntity){
		window.location = "/ermsweb/viewSubAccountsUpdate?acctId="+acctId+"&subAcc="+subAcc+"&bizUnit="+bizUnit+"&dataSourceAppId="+dataSourceAppId+"&bookEntity="+bookEntity;
	}
...
<body>
	<div class="boci-wrapper">
		...
		<div class="page-title">Sub Account RMD Maintenance - View </div>
		<!-- <input type="hidden" id="pagetitle" name="pagetitle"
			value="Sub Account RMD Maintenance - View "> -->
		<div id="account-details-section">
			...
			<table id="account-details-section-table" cellpadding="6"
				...
				<tr>
					<td colspan="4"
						style="background-color: #393052; color: white; width: 500; font-size: 13px">Sub
						Account RMD Detail section</td>
				</tr>
				<tbody>
					<tr>
					...
					<tr>
						<td style="font-size: 13px; background-color: #B5A2C6">
							Account - Name.</td>
