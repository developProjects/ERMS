...
<html lang="en">
 ...
 <meta http-equiv="x-ua-compatible" content="IE=10">
    	...
    	var dataSource = new kendo.data.DataSource({
			transport: {
				read: {
					...
					url: window.sessionStorage.getItem('serverPath')+"countryRating/getRecord?countryId="+countryId+"&userId="+window.sessionStorage.getItem("username"),
					dataType: "json",
			...
			dataSource.fetch(function(){
				...
				$.each(jsonObj, function(i){
						...
						$(".command-button-Section").html("<button id=\"editBtn\"  type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\" onclick=\"toEdit('"+jsonObj[i].countryId+"')\">Update</button></label> <label><button id=\"deleteBtn\"  type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\" onclick=\"toDelete('"+jsonObj[i].countryId+"')\">Delete</button></label> <label><button id=\"backBtn\" onclick=\"toBack()\" type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\">Back</button>");
	...
	function toEdit(countryId){
		window.location = "/ermsweb/countryRatingUpdate?countryId="+countryId;
	}
	function toDelete(countryId){
		window.location = "/ermsweb/countryRatingDelete?countryId="+countryId;
	}
	function toBack(){
		//window.history.back();
    ...
    <body>
    	<div class="boci-wrapper">
    		...
    		<div class="content-wrapper">
					<div class="page-title">Maintenance  - Country Rating Maintenance (Collateral) - View Country Rating Detail (Collateral)</div>	    		
