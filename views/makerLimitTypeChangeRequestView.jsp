...
<html lang="en">
 ...
 <meta http-equiv="x-ua-compatible" content="IE=10">
  ...
  <!--   <script src="/ermsweb/resources/js/jszip.js"></script> -->
    	...
    	var dataSource = new kendo.data.DataSource({
			...
			dataSource.fetch(function(){
				...
				$.each(jsonObj, function(i){
					...
					$("#verifierRemarks").html(jsonObj[i].status);
					$(".command-button-Section").html("<button id=\"editBtn\"  type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\" onclick=\"toEdit('"+ jsonObj[i].lmtTypeCode+ "','"+jsonObj[i].crId+"')\">Edit</button><label>    </label><button id=\"discardBtn\"  type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\" onclick=\"toDiscard('"+ jsonObj[i].lmtTypeCode+ "','"+jsonObj[i].crId+"')\">Discard</button><label>    </label><button id=\"backBtn\" onclick=\"toBack()\" type=\"button\" data-role=\"button\" class=\"k-button\" role=\"button\" aria-disabled=\"false\" tabindex=\"0\">Back</button>");
    ...
    function toEdit(eid,crId){
		//window.history.back();
    	window.location = "/ermsweb/makerLimitTypeChangeRequestUpdate?lmtTypeCode=" + eid+"&crId="+crId;
	}
    function toDiscard(did,crId){
		//window.history.back();
	
    	window.location = "/ermsweb/makerLimitTypeChangeRequestDiscard?lmtTypeCode=" + did+"&crId="+crId;
	}
