...
<script>
		$(document).ready(
			function(){
				...
				var dataSource = new kendo.data.DataSource({
					transport: {
						read: {
							url: window.sessionStorage.getItem('serverPath')+"subacct/getRecord?crId="+array[0]+"&subAcc="+getURLParameters("subAcc")+"&bizUnit="+array[1]+"&bookEntity="+array[2]+"&dataSourceAppId="+array[3]+"&userId="+window.sessionStorage.getItem("username"),
							dataType: "json",
			...
			dataSource.fetch(function(){
				...
				var data = [
					{ text: tmpAcctClientType, value: tmpAcctClientType },
					{ text: "AAA", value: "BBB" },
					{ text: "BBB", value: "BBB" },
					{ text: "DDD", value: "DDD" },
					{ text: "EEE", value: "EEE" }
				...
				$("#saveBtn").kendoButton({
        			click: function(){
    					var updateCriteria = {
    						...
    						subAcctId: getURLParameters("subAcc"),
    						bizUnit: array[1],
	    				...
	    				dataDatasource.set("subAcctId",updateCriteria.subAcctId);
		...
		function toDiscard(uid){
			window.location = "/ermsweb/discardSubAccounts?crId="+uid+"&userId="+window.sessionStorage.getItem("username");
		}
...
<body>
	<div class="boci-wrapper">
		...
		<div class="page-title">Sub Account RMD Change Request Maintenance - Update</div>
		<!-- <input type="hidden" id="pagetitle" name="pagetitle"
			value="Sub Account RMD Change Request Maintenance - Update "> -->
