    ...
    $(document).ready(function(){
    	...
    	var dataSource = new kendo.data.DataSource({
			transport: {
				...
				update:{
					type:"POST",
					url: window.sessionStorage.getItem('serverPath')+"lmtMonitorRule/verify",
					dataType: "json",
			...
			dataSource.fetch(function(){
				...
				$("#verifyBtn").kendoButton({
	    			click: function(){
	    				...
	    				dataSource.transport.options.update.url= window.sessionStorage.getItem('serverPath')+"lmtMonitorRule/verify";
	    				dataDatasource.set("crStatus","verified");
	    				dataDatasource.set("crRemarks",$("#verify_remarks").val());
	    				dataSource.sync();
	    			}
	    		});
	      		...
	      		$("#returnBtn").kendoButton({
	    			click: function(){    				
	    				var dataDatasource = dataSource.at(0);
	    				dataSource.transport.options.update.url= window.sessionStorage.getItem('serverPath')+"lmtMonitorRule/return";
    		...
    		<div class="viewLimitTypeDetails-content-wrapper form">
	    			...
	    			<div class="approval-section margin-top">
						<div id="approval-section-header">Approval</div>
							...
							<tr>
								<td class="field-label bold">Action performed by Maker:</td>
								<td><label class="bold">Create/Update/Delete of Monitor Rule</label></td>
							</tr>
							<tr>
								<td style="vertical-align:top" class="bold">Remarks</td>
								<td>
									<textarea id="verify_remarks" class="remarks-textarea"></textarea>
								</td>
							</tr>			
						</table>
					</div>
